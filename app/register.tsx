...
const ROLES: { id: Role; label: string; icon: string; description: string }[] = [
  {
    ...
    label: 'Kliyan',
    icon: '📍',
    description: 'Mande moto-taxi oswa livrezon',
  },
  {
    id: 'chauffeur',
    label: 'Chofè Moto-Taxi',
    icon: '🏍️',
    description: 'Pran pasaje, touche lajan',
  },
];
...
const ROLE_INFO: Record<string, { color: string; bg: string; message: string }> = {
  client: {
    ...
    bg: '#D6F5E3',
    message:
      '🗺️ Ou pral gen aksè a yon map pou jwenn chofè pre ou, suivi kòmand an tan reyèl, ak pèman fasil via MonCash oswa NatCash.',
  },
  chauffeur: {
    color: '#7A3D00',
    bg: '#FFF0D6',
    message:
      '🏍️ Ou pral wè demann pasaje alantou ou, jere kous ou yn. Yon tablo de bò chofè pral disponib jwenn lis livrezon disponib nan zòn ou, ak yon entèfas espesyal pou jere kòmand restoran ak boutik.',
  },
};
...
export default function Register() {
  ...
  function validate() {
    if (!nom.trim() || !prenom.trim()) {
      Alert.alert('Erè', 'Tanpri antre non ak prenon ou.');
      return false;
    }
    if (!email.includes('@')) {
      Alert.alert('Erè', 'Adrès email pa valid.');
      return false;
    }
    if (telephone.length < 8) {
      Alert.alert('Erè', 'Nimewo telefòn dwe gen omwen 8 chif.');
      return false;
    }
    ...
    if (password !== confirmPassword) {
      Alert.alert('Erè', 'Modpas yo pa matche.');
      return false;
    }
    if (!role) {
      Alert.alert('Erè', 'Tanpri chwazi yon wòl.');
      return false;
    }
    if (!acceptConditions || !acceptPrivacy) {
      Alert.alert('Erè', 'Ou dwe aksepte kondisyon itilizasyon ak politik konfidansyalite yo.');
      return false;
    }
    return true;
  }
  ...
  async function sInscrire() {
    ...
    Alert.alert(
      'Bienveni! 🎉',
      'Kont ou kreye ak siksè. Verifye email ou pou konfime.',
    ...
    >
      ...
      >
        ...
        <View style={styles.form}>
          ...
          <TextInput
            style={styles.input}
            placeholder="Nimewo Telefòn (ex:3700 0000)"
            placeholderTextColor="#AAA"
            value={telephone}
          ...
          <TextInput
            style={styles.input}
            placeholder="Vil (ex: Pòtoprens, Delmas, Pétion-Ville...)"
            placeholderTextColor="#AAA"
            value={ville}
          ...
          {/* ── Section: Rò ── */}
          <SectionLabel>🎭 Chwazi Rò Ou</SectionLabel>

          <View style={styles.roleNote}>
            <Text style={styles.roleNoteText}>
              ⚠️ <Text style={{ fontWeight: '700' }}>Enpòtan :</Text> Entèfas aplikasyon an ak fonksyon yo pral <Text style={{ fontWeight: '700' }}>chanje selon wòl ou chwazi a.</Text> Chwazi avèk atansyon.
            </Text>
          </View>
          ...
          {ROLES.map(r => (
            <TouchableOpacity
              key={r.id}
              style={[styles.roleCarte, role === r.id && styles.roleCarteActive]}
          ...
          <CheckRow
            ...
            onToggle={() => setAcceptConditions(v => !v)}
          >
            <Text style={styles.checkText}>
              Mwen aksepte{' '}
              <Text
                style={styles.link}
                onPress={() => Linking.openURL('https://taxipam.app/conditions')}
              >
          ...
          >
            <Text style={styles.checkText}>
              ...
              <Text
                style={styles.link}
                onPress={() => Linking.openURL('https://taxipam.app/privacy')}
              >
          ...
          <TouchableOpacity onPress={() => router.push('/login')} style={styles.loginLink}>
            <Text style={styles.loginLinkText}>
              ...
              <Text style={styles.link}>Konekte isit</Text>
            </Text>
          </TouchableOpacity>
      ...
      <View style={pwStyles.bars}>
        {[0, 1, 2, 3].map(i => (
          <View
            ...
            style={[
              pwStyles.bar,
              { backgroundColor: i < score ? colors[score - 1] : '#E0D8C8' },
            ]}
          />
        ))}
      </View>
      <Text style={[pwStyles.label, { color: colors[score > 0 ? score - 1 : 0] }]}>
        {labels[score]}
      </Text>
    </View>
  );
